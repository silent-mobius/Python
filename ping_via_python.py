...
class Pinger(object):
	...
	def send_ping(self, sock, ID):
		...
		my_checksum = 0  # Create a dummy header with a 0 checksum.
	...
	try:
		...
		if errno == 1 :# Not superuser, so operation not permitted
	...
	def ping(self):
		...
		try:
			...
			break
		if delay == None:
			print (f'Ping failed. (timeout within { \ \} secs')
			self.timeout
		else:
			delay = delay * 1000
			print (f'Get pong in {delay}')
